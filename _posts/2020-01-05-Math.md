...
When X is a binary indicator taking values 0 and 1, E(X) = P(X = 1).
...
If Y is a discrete random variable, then $$E(Y|X=x) = \sum_{y}^{}yP(Y=y)|X=x$$

If Y is a continuous random variable, then $$E(Y|X=x) = \int_{y}{}yf(y|x)dy$$
