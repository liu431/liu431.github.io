...
The coefficients in a least squares model typically are interpreted as the expected change in the outcome of interest associated with a unit change in the predictors, $$Y = \beta_{0}+\beta_{1}X_{1}+...+\beta_{p}X_{p}+\epsilson$$
...
such as $$\frac{\partial Y}{\partial X_{1}} = \beta_{1}+\beta{2}X_{1}$$ or $$\frac{\partial Y}{\partial X_{1}} = \beta_{1}+2\beta{2}X_{1}$$ 
